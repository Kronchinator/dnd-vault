---
name: Ressi Res Essiressires Lord Master Doctor
player: 
race: Kalashtar
class: Cleric
level: 1
subclass: Nature Domain
background: 
tags: [player-character, cleric, kalashtar]
---

# Ressi Res Essiressires Lord Master Doctor
*Level 1 Kalashtar Cleric*

---

## Combat Stats

| | |
|--|--|
| **AC** | 17 (scale mail 14 + DEX 1 + shield 2) |
| **HP** | 9 (8 + CON 1) |
| **Hit Die** | d8 |
| **Initiative** | +1 (DEX) |
| **Speed** | 30 ft |
| **Size** | Medium |

> ⚠️ Stat correction from original sheet: WIS 18 = **+4** (not +3). This affects spell DC and attack.

## Ability Scores

| Stat | Score | Modifier |
|------|-------|----------|
| STR | 13 | +1 |
| DEX | 12 | +1 |
| CON | 13 | +1 |
| INT | 10 | +0 |
| WIS | 18 | +4 |
| CHA | 10 | +0 |

**Proficiency Bonus:** +2

### Saving Throws
- Wisdom (proficient) +6
- Charisma (proficient) +2

### Skills
| Skill | Mod |
|-------|-----|
| Religion | +2 |
| Insight | +6 |
| Medicine | +6 |

---

## Racial Traits (Kalashtar)

- **Mental Discipline** — Resistance to psychic damage
- **Dual Mind** — Advantage on Wisdom saving throws
- **Mind Link** — As an action, telepathically link with one willing creature within 60 ft (same language not required; can share emotions and basic images). Lasts until the end of your next turn.
- **Severed from Dreams** — Immune to spells and effects that require the target to dream (e.g. *Dream* spell). Still needs sleep normally for long rests.
- **Languages** — Common, Quori, and one additional language

---

## Abilities

- **Spellcasting:** WIS-based
  - **Spell Save DC:** 14 (8 + 2 prof + 4 WIS)
  - **Spell Attack Bonus:** +6
  - **Prepared Spells per day:** 5 (WIS mod 4 + level 1) + domain spells always prepared
- **Acolyte of Nature** *(Nature Domain, level 1)* — Gain one druid cantrip of choice (not yet selected). Gain proficiency in one of: Animal Handling, Nature, or Survival (not yet selected).
- **Nature Domain Spells** — Always prepared, don't count against prepared spell limit:
  - Level 1: Animal Friendship, Speak with Animals ✓
  - Level 3: Barkskin, Spike Growth
  - Level 5: Plant Growth, Wind Wall
  - Level 7: Dominate Beast, Grasping Vine
  - Level 9: Insect Plague, Tree Stride
- [[Channel Divinity - Charm Animals and Plants]] *(unlocks at level 2)* — As an action, present holy symbol; each beast and plant within 30 ft must succeed WIS save (DC 14) or be charmed for 1 minute.

---

## Spells

**Cantrips**
- [[Sacred Flame]]
- [[Spare the Dying]]
- [[Thaumaturgy]]

**Domain Spells** *(always prepared)*
- [[Speak with Animals]]
- [[Animal Friendship]]

**1st Level Slots:** 2
*(Additional prepared spells not yet specified — up to 5 per day from full cleric list)*

---

## Equipment

| Item | Notes |
|------|-------|
| Mace | 1d6 bludgeoning |
| Greatclub | 1d8 bludgeoning, two-handed |
| Scale mail | AC 14 + DEX (max +2) |
| Shield | +2 AC |
| Holy Symbol | Spellcasting focus |
| Priest's Pack | Backpack, blanket, 10 candles, tinderbox, alms box, 2 incense blocks, censer, vestments, 2 days rations, waterskin |

---

## Backstory

*Not yet provided.*

---

## DM Notes

- **Acolyte of Nature choices not yet made** — needs one druid cantrip and one skill proficiency (Animal Handling, Nature, or Survival).
- Carries both a Mace and a Greatclub — the Greatclub requires two hands and cannot be used with a shield. Clarify intended combat loadout.
- WIS 18 at level 1 is exceptional — DC 14 saves and +6 spell attack will be dominant at this tier.
- Animal Friendship (domain spell, always prepared) not yet noted on original sheet — remind player.
